...
archivos = "Files" # Carpeta de archivos html
archivosfix = "FilesFix" # Carpeta para almacenar archivos html sin etiquetas
archivosList = "FilesList" # Carpeta para almacenar lista de texto de los html
archivosToken = "FilesToken" # Carpeta para almacenar listas tokenizadas
...
def open_file():
    ...
    for filename in os.listdir(archivos):
        start = time.time()
        with open(os.path.join(archivos, filename), "r") as f:
            end = time.time()
            total+= (end - start)
        ...
        with open(os.path.join(archivosfix, filename), "r") as f:
            ...
            with open(os.path.join(archivosList, filename), 'w+') as outfile:
                    for lines in f:
                        ...
                        line = re.sub(r"[^\w\s]","", line) # Remover caracteres especiales
...
def Ascii():
    ...
    mytable = PrettyTable(["Token","Repeticiones","# de archivos con ese token"])
        ...
        with open(os.path.join(archivosToken, filename), "r") as f:
            ...
            with open(os.path.join(archivosList, filename), 'r') as outfile:
                    for word in f: # Saber la cantidad de archivos que contienen una misma palabra 
                        word = re.sub(r"(\d+)", '', word)#SEGUNDO NUMERO ES EL VALOR
                        word = re.sub(r"\n","", word)
    ...
    with open("Token_List.txt","w") as fin:
        for key, value in countTotal.items():
            value = str(value)
            fin.write(key+" "+value+"\n")
